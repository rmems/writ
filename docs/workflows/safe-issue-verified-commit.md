...
- Owner is outside the configured allowlist (unless the operator named this repo/job explicitly). **Partial:** `writ watchlist check-all` reads `WRIT_ALLOWED_OWNERS`. Other discovery/scheduling still has no reader under `crates/` (#146).
...
3. Create or reuse a dedicated branch and isolated worktree:
   ...
   - What routing through `writ-core` actually gets you, so the promise is not larger than the code: the git/gh argv allowlist, no merge path, force-with-lease only, sandboxed path derivation with symlink rejection, exact-base resolution, branch and `HEAD` postcondition verification, supervised child processes, origin-slug matching for `gh -R`, and `WRIT_ALLOWED_OWNERS` for `writ watchlist check-all`. **Owner-allowlist enforcement is not a general discovery/worktree gate.** Broader scheduling still has no reader (#146); it was enforced in the Python layer removed by [#144](https://github.com/rmems/writ/issues/144).
