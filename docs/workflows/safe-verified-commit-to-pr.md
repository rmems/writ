...
- Owner outside the configured allowlist unless the operator named this job. `writ claim` already enforced this at isolate time when `WRIT_ALLOWED_OWNERS` was set; this later stage does not re-check. Multi-owner discovery still has no reader (#146).
