...
> [!NOTE]
> **Status: Phase 1 hook dispatcher is in this tree.** `writ hook` admits `PreToolUse` (Bash `git`/`gh` only), `WorktreeCreate`, and `WorktreeRemove`, and `writ install` writes the hook block. The SQLite lease store records durable worktree ownership for verified reclaim ([#141](https://github.com/rmems/writ/issues/141)): grant/release plus reserved budget columns, not MCP or budget enforcement. Live Claude Code end-to-end burn-in is still outstanding on [#124](https://github.com/rmems/writ/issues/124).
...
Leases are the join: coordination state that the enforcement layer checks at write time. Phase 1 ships a SQLite skeleton (`leases.db`) so create/remove is not a Markdown-only control plane, including verified reclaim ([#141](https://github.com/rmems/writ/issues/141)). Budget columns are reserved and unused; enforcement is [#167](https://github.com/rmems/writ/issues/167).
