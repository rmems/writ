...
Leases are the join: coordination state that the enforcement layer checks at write time. `writ worktree create` now records a crash-consistent lease before git mutation; the hook dispatcher that will make that check unbypassable is still M1.
