...
- Process timeouts terminate and reap children. Hang recovery is graceful cancel (Unix `SIGTERM`) then process-group kill, with optional stall detection and stderr progress heartbeats — see [`docs/supervisor-timeouts.md`](docs/supervisor-timeouts.md). This is implemented in `supervisor.rs` and `timeout_policy.rs` (wall-clock timeout inclusive of permit wait, process-group kill on drop on Unix -- Windows kills only the direct child and has no graceful-cancel equivalent -- and wrapper/interpreter rejection); changes to it require timeout, stall, reaping, and residual tests, not a deferral. Recovery must not merge, bare-force-push, or emit a commit SHA.
