...
- State writes use a temporary file and atomic replacement without leaving partial JSON. `state.rs` remains a `watched.json` read path only. Durable writes now belong to the SQLite lease store (`leases.db`): allocation identity is prepared before git mutation, and released/tombstoned rows must not be resurrected (#136).
